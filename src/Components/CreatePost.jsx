...
function UploaderImagePost() {
  ...
  return (
    <article>
      <div
        onClick={
          () => document.querySelector('.inptuImageCreatePost').click()
        }
        ...
        <input
          ...
          type="file" accept="image/*" className="inptuImageCreatePost" hidden />
        {
          image ?
            <img
...
export function CreatePost({ onCreatePost }) {
  ...
  const handleSubmit = async (e) => {
    ...
    axios.post('http://localhost:1234/post', data, {
      headers: {
  ...
  return (
    <article className="w-full px-4 max-w-[750px] gap-4 flex flex-col">
      <div className="flex gap-4 justify-between items-center bg-white py-4 px-6 rounded-md basis-[100%] border-2 dark:bg-zinc-900 dark:border-zinc-800">
        ...
        <button onClick={onOpen} className="w-full bg-slate-200 dark:bg-zinc-800 rounded-md py-3 pl-4 text-left dark:text-zinc-200 font-semibold">¿Que estas pensando?</button>
        <Modal isOpen={isOpen} placement="center" onOpenChange={onOpenChange} isDismissable={false} backdrop="blur">
          <ModalContent>
            {(onClose) => (
              <form onSubmit={handleSubmit} >
                <ModalHeader className="flex flex-col gap-1">Crear Publicacion</ModalHeader>
                <ModalBody>

                  {
                    isSending ? <Spinner label="Loading..." color="primary" labelColor="primary" /> : null
                  }
